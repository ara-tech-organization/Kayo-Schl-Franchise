...
function track(event, detail) {
  ...
  if (typeof window !== 'undefined' && window.dataLayer) window.dataLayer.push({ event, ...detail })
...
export default function QualForm({ idPrefix = 'q' }) {
  ...
  const submit = (e) => {
    ...
    if (Object.keys(next).length) return
    setDone(true)
    track('qualified_lead', { capital: values.capital, timeline: values.timeline })
  }
    ...
    return (
      <div className="thanks">
        ...
        <p style={{ fontSize: 14.5, color: 'var(--muted)', marginTop: 8 }}>
          The franchise desk will confirm your city's availability and call you <b>within 1 working day</b>.
          Want to move faster?
        </p>
      ...
      <form onSubmit={submit} noValidate>
        {step === 1 ? (
          <div className="fstep">
            ...
            <button type="button" className="btn btn-primary" onClick={toStep2} data-cta="form-step1">
              Continue — Check My City
              <Icon name="arrow-right" size={18} />
            </button>
            <p className="privacy">
              <Icon name="lock" size={13} />
              Your details go only to the Kayo franchise desk. No third-party sharing.
            </p>
          </div>
        ) : (
          <div className="fstep">
            ...
            <button type="submit" className="btn btn-lime" data-cta="form-submit">
              Submit My Franchise Profile
              <Icon name="arrow-right" size={18} />
            </button>
            <p className="privacy">
              By submitting you agree to be contacted by Kayo's franchise team about this enquiry.
            </p>
          </div>
        )}
      </form>
